...
from mistune import escape_url, escape_html, escape
...
def parse_options(rest: str, code: str) -> dict:
    ...
    if 'linenos' in options:
        rv['linenos'] = options.pop('linenos').lower() not in ('n', 'no', 'false', 'off')
...
class MyRenderer(mistune.HTMLRenderer):
    ...
    def _block_code_info(self, code, info):
        ...
        html = html.replace(
            '<pre>', 
            '<pre class="code" data-lang="' + escape_html(data_lang) + '">',
            1
        )
...
def render_html_theading(text, level, tid):
    ...
    headerlink = (
        '<span class="headerlink"> <a href="#' + escape_url(tid) + 
